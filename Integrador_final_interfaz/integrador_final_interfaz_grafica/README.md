...
Datos guardados temporamente a los efectos de listar Reportes
 ...
 ⚙️ Funcionalidades CRUD por consola e interfaz, la idea era que ambos tubieran la misma funcionalidad pero no llegué por los inconvenientes de conectividad que aun persisten.
...
*Búsqueda de un producto, el valor del orecio unitario, stock, descriociòn o categorìa por id/nombre.
...
*Eliminar producto por nombre e ID, mostrando el producto para eliminarse, y guardandolo en la tabla productos eliminados.
...
En las actividades que se se muestra un producto sólo con su descripción, y una tabla de todos los productos con sus descripciones, para que el texto de la misma no ocupe todo el espacio horizontal, procedo con una función a "envolver" el texto dentro de un determinado número de caracteres así se conserva las columnas de la tabla del listado.
...
Instalar dependencias (matplotlib, regex, colorama, tabulate, datatime, numpy, pandas, tkinter):
...
*** La intencion es que el mismo CRUD que se hace por consola con graficas etc se hiciera con la interfaz Gr+afica.